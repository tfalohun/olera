...
export default async function ProviderPage({
  ...
  // Olera Score: 5-point scale
  const oleraScore = rating ? Math.round(rating * 10) / 10 : null;
            ...
            {oleraScore && (
              <div className="text-center mb-16">
                ...
                <div className="flex items-center justify-center gap-1 mb-8">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <StarIcon key={star} className={`w-5 h-5 ${star <= Math.round(oleraScore) ? "text-yellow-400" : "text-gray-200"}`} filled={star <= Math.round(oleraScore)} />
                  ))}
                </div>
                {/* Breakdown cards */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                  {[
                    { label: "Community", value: Math.min(5, oleraScore + 0.1) },
                    { label: "Value", value: Math.min(5, oleraScore - 0.1) },
                    { label: "Transparency", value: Math.min(5, oleraScore - 0.3) },
