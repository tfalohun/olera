    ...
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full pl-4 pr-20 py-3 text-[15px] text-gray-900 placeholder-gray-400 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-300 transition-all"
            />
            <button className="absolute right-1.5 top-1/2 -translate-y-1/2 px-4 py-1.5 text-[13px] font-semibold text-white bg-primary-600 rounded-md hover:bg-primary-700 transition-colors">
              Ask
            </button>
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {suggestedQuestions.slice(0, 3).map((q) => (
                <button
                  ...
                  className="text-[13px] text-gray-500 hover:text-primary-600 px-3 py-1.5 bg-white border border-gray-200 rounded-full hover:border-primary-200 transition-colors"
                >
                  {q}
                </button>
              ))}
      ...
      {visibleQuestions.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {visibleQuestions.map((qa, index) => {
            ...
            return (
              <div key={index} className="bg-gray-50 rounded-xl p-5 flex flex-col">
                {/* Questioner */}
      ...
      {hasMore && (
        <button
          ...
          className="mt-4 text-[14px] font-semibold text-primary-600 hover:text-primary-700 flex items-center gap-1.5 transition-colors"
        >
